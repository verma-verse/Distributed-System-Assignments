...
void request(int pid) {
	...
	if (token_holder == pid) {
		...
		site[pid].exec = 1;
		printf(
			"SITE % d already has the token and it enters the critical "
			"section\n ",
			pid);
		return;
	}
	...
	if (token_holder == pid) {
		...
		printf(
			"SITE % d is currently executing the critical section \nSite %d "
...
void print() {
	...
	for (i = 0; i < PR; i++) printf("% d ", token.ln[i]);
	printf("\n");
	printf("SITES SEQ NO ARRAY : \n");
	for (i = 0; i < PR; i++) {
		printf(" S % d :", i);
		for (j = 0; j < PR; j++) printf(" % d ", site[i].rn[j]);
		printf("\n");
	}
}
